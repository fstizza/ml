...
/* ---------------------------------------------------------------------------------- */
int arquitec(char *filename, struct KNN *nb, struct DATOS *datos)
{
  FILE *b;
  char filepat[100];
  int i, j;
...
/* ------------------------------------------------------------------------------ */
/* idx: retorna el indice del primer vecino vacio o el que de mayor distancia
   (que sera reemplazado) en caso de ser mayor que el nuevo dato presentado.
/* ------------------------------------------------------------------------------ */
int idx(Neighbour *neighbours, int k)
{
  float max_dist = -1;
  int id = -1;

  for (size_t i = 0; i < k; i++)
    if (neighbours[i]->clase == -1)
      return i;

  for (size_t i = 0; i < k; i++)
    if (neighbours[i]->dist > max_dist)
    {
      max_dist = neighbours[i]->dist;
      id = i;
    }
...
/* ------------------------------------------------------------------------------ */
/* resolverEmpate: resuelve un empate.
/* ------------------------------------------------------------------------------ */
int resolverEmpate(int *clases, int nin, int cant_empatadas)
{
  int empatadas[cant_empatadas];  

  // Copiamos las clases que empataron:
  int j = 0;
  for (int i = 0; i < nin && j < cant_empatadas; i++)
  {
    if (clases[i])
    {
      empatadas[j] = i;
  ...
  srand(time(NULL));
  int num = rand() % cant_empatadas;
  if(CONTROL > 2) printf("\nEmpate resuelto por rand: %d\n", empatadas[num]);
  return empatadas[num];
}

/* ------------------------------------------------------------------------------ */
/*output: calcula la clase dado un vector de entrada *input
/* ------------------------------------------------------------------------------ */
int output(KNN nb, float *input, DATOS datos)
{
  Neighbour neighbours[nb->K];
  for (size_t i = 0; i < nb->K; i++)
  {
    neighbours[i] = (Neighbour)malloc(sizeof(Neighbour));
    neighbours[i]->clase = -1;
    neighbours[i]->dist = -1.0f;
  }

  // Buscamos los N vecinos mas cercanos.
  for (size_t i = 0; i < datos->PR; i++)
  {
    float distancia = dist(input, datos->data[i], datos->N_IN);

    int id_max_dist_neig = idx(neighbours, nb->K);

    if (neighbours[id_max_dist_neig]->clase == -1 || distancia < neighbours[id_max_dist_neig]->dist)
    {
      neighbours[id_max_dist_neig]->dist = distancia;
      neighbours[id_max_dist_neig]->clase = datos->data[datos->seq[i]][datos->N_IN];
    }
  }

  int clases[nb->N_Class];
  for (size_t i = 0; i < nb->N_Class; i++) clases[i] = 0;

  // Contamos las ocurrencias de las clases de los vecinos mas cercanos.
  for (size_t i = 0; i < nb->K; i++) clases[neighbours[i]->clase] += 1;

  // Realizamos la vocation
  int clase = 0, votacion = -1;
  for (size_t i = 0; i < nb->N_Class; i++)
    if (clases[i] > votacion)
    {
      votacion = clases[i];
      clase = i;
    }

  // Reutilizo el vector clases para ver si hay empates.
  for (size_t i = 0; i < nb->N_Class; i++)
    clases[i] = clases[i] == votacion;

  int cant_iguales = 0;
  for (size_t i = 0; i < nb->N_Class; i++) if (clases[i]) cant_iguales++;

  if(cant_iguales > 1) return resolverEmpate(clases, nb->N_Class, cant_iguales);
...
/* ------------------------------------------------------------------------------ */
float evaluar(struct KNN *nb, struct DATOS *datos, float **S, int pat_ini, int pat_fin, int usar_seq)
{
  ...
  for (patron = pat_ini; patron < pat_fin; patron++)
  {
    ...
    datos->pred[nu] = output(nb, S[nu], datos);
...
int train(KNN nb, DATOS datos)
{
  ...
  // Hay que evaluar esto? Como se obtiene el error de entrenamiento?
  ...
  sprintf(filepat, "%s.kpred", datos->filename);
