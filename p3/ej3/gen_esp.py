...
def nb_train_parameter(prefix):
    ...
    val_err = []
    for bin in bins:
        fname = f'{prefix}_{bin}_espiralesanidadas'
        os.system(f'cp ./datasets/espiralesanidadas.data ./datasets/{prefix}/{fname}.data')

        os.system(f'cp ./datasets/synth.nb2 ./datasets/{prefix}/{fname}.nb2')

        os.system(f'cp ./datasets/espiralesanidadas.test ./datasets/{prefix}/{fname}.test')
...
def main():
    ...
    ax = errors.plot(figsize=(8, 6), title='Espirales anidadas', style=['.--', '.--', '.--'], color=[
                     '#cc00ff', '#d40000', '#0099cc'], xticks=list(range(10, 641, 20)).insert(0, 5))
    ax.set_xlabel('Bins')
    ax.set_ylabel('Error porcentual')
    ax.legend(loc='upper left')

    plt.savefig(f'img/nb-err-hist-espiralesanidadas.png')
    plt.close()
