...
// estructura que representa a un bin del histograma
typedef struct _Bin
{
  int freq;
  float min;
...
struct NB
{
  ...
  int N_BINS;  // Cantidad de bins en el histograma
  ...
  int *clases;       // Cantidad de elementos que tiene cada clase
  Bin ***histograma; // Histograma de probabilidades
};
...
int define_matrix_nb(struct NB *nb)
{
  ...
  nb->clases = (int *)calloc(nb->N_Class, sizeof(int));
  nb->histograma = (Bin ***)calloc(nb->N_Class, sizeof(Bin **));

  for (i = 0; i < nb->N_Class; i++)
  {
    nb->histograma[i] = (Bin **)calloc(nb->N_IN, sizeof(Bin *));
    for (j = 0; j < nb->N_IN; j++)
    {
      nb->histograma[i][j] = (Bin *)calloc(nb->N_BINS, sizeof(Bin));
    }
  }
...
float prob(struct NB *nb, float x, int feature, int clase)
{
  ...
  {
    if (x < nb->histograma[clase][feature][i].max)
    {
      break;
    }
  }

  prob = (nb->histograma[clase][feature][i].freq + (1 / nb->N_BINS)) / ((float)nb->clases[clase] + 1.0);
...
/* ------------------------------------------------------------------------------ */
int output(struct NB *nb, float *input, struct DATOS *datos)
{
  int i, k;
  float prob_de_clase;
  float max_prob = -1e40;
  ...
  for (k = 0; k < nb->N_Class; k++)
  {
    ...
    prob_de_clase += log((float)nb->clases[k] / datos->PR);
...
int train(struct NB *nb, struct DATOS *datos)
{
  ...
  //Leemos cada patron de los datos para obtener la cantidad de puntos de cada clase
  for (i = 0; i < N_TOTAL; i++)
  {
  ...
  for (i = 0; i < nb->N_Class; i++)
  {
    printf("\nCantidad de patrones de clase %d: %d (%lf%%)\n", i, nb->clases[i], (((float)nb->clases[i]) / N_TOTAL) * 100.);
  }
  ...
  for (i = 0; i < nb->N_IN; i++)
  {
    ...
    for (j = 0; j < N_TOTAL; j++)
    {
      ...
      if (datos->data[datos->seq[j]][i] < min)
      {
        min = datos->data[datos->seq[j]][i];
      }
      if (datos->data[datos->seq[j]][i] > max)
      {
        max = datos->data[datos->seq[j]][i];
      }
    }
    ...
    for (j = 0; j < nb->N_Class; j++)
    {
      ...
      {
        nb->histograma[j][i][k].size = size;
        nb->histograma[j][i][k].min = min + k * size;
        nb->histograma[j][i][k].max = nb->histograma[j][i][k].min + size;
        nb->histograma[j][i][k].freq = 0;
      }
    }
  }
  ...
  for (i = 0; i < N_TOTAL; i++)
  {
    ...
    for (j = 0; j < nb->N_IN; j++)
    {
      for (k = 0; k < nb->N_BINS; k++)
      {
        if (datos->data[datos->seq[i]][j] < nb->histograma[clase][j][k].max)
        {
          break;
        }
      }
      nb->histograma[clase][j][k].freq++;
    }
  }
