...
float pesos_cuadrados(float **w1, float **w2, int n1, int n2, int n3)
{
  float weight = 0.0f;
  size_t i, j;
...
int train(struct MLP *nn, struct DATOS *datos)
{
  ...
  float h;
  float eta, suma;
  float mse, mse_train, mse_valid, mse_test, minimo_valid, penalizacion, wd_mse;
  ...
  penalizacion = nn->GAMMA != 0 ? 1.0f - (2.0f * nn->GAMMA * nn->ETA) : 0.0f;
  ...
  for (iter = 1; iter <= nn->ITER; iter++)
  {
    ...
    for (nu1 = 0; nu1 < datos->PR; nu1++)
    {
      ...
      for (i = 1; i <= nn->N3; i++)
        for (j = 0; j <= nn->N2; j++)
        {
          ...
          if (penalizacion) nn->w2[i][j] *= penalizacion; /* penalizacion wd */
      ...
      for (j = 1; j <= nn->N2; j++)
        for (k = 0; k <= nn->N1; k++){
          ...
          if (penalizacion) nn->w1[i][j] *= penalizacion; /* penalizacion wd */
    ...
    wd_mse += pesos_cuadrados(nn->w1, nn->w2, nn->N1, nn->N2, nn->N3);
