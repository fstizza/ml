...
struct MLP
{
  ...
  float GAMMA;          /* WEIGHT DECAY: termino de penalizacion
...
void sinapsis_rnd(float max, struct MLP *nn)
{
  ...
  for (j = 1; j <= nn->N2; j++)
    for (i = 0; i <= nn->N1; i++)
      nn->w1[j][i] = unif_rnd(max);

  /*segunda capa*/
  for (j = 1; j <= nn->N3; j++)
    for (i = 0; i <= nn->N2; i++)
      nn->w2[j][i] = unif_rnd(max);
}
/* ------------------------------------------------------------------- */
...
int sinapsis_read(int numero, struct MLP *nn)
{
  ...
  for (j = 1; j <= nn->N2; j++)
    for (i = 0; i <= nn->N1; i++)
      fscanf(fp, "%f", &nn->w1[j][i]);

  /*segunda capa*/
  for (i = 1; i <= nn->N3; i++)
    for (j = 0; j <= nn->N2; j++)
      fscanf(fp, "%f", &nn->w2[i][j]);
...
int define_matrix_mlp(struct MLP *nn)
{
  ...
  if (nn->x1 == NULL || nn->x2 == NULL || nn->x3 == NULL || nn->grad2 == NULL || nn->grad3 == NULL || nn->target == NULL)
    return 1;
...
int arquitec(char *filename, struct MLP *nn, struct DATOS *datos)
{
  ...
  // WEIGHT DECAY - Termino de penalizacion
  fscanf(b, "%f", &nn->GAMMA);
  fclose(b);
  printf("GAMMA: %.8f\n", nn->GAMMA);
  ...
  printf("\nTermino de penalizacion: %.8f", nn->GAMMA); // WEIGHT DECAY
  if (CONTROL)
  {
    printf("\nSINAPSIS:\n");
    for (j = 1; j <= nn->N2; j++)
  ...
  for (patron = pat_ini; patron < pat_fin; patron++)
  {
    ...
    for (i = 1; i <= nn->N3; i++)
    {
      *mse += (nn->x3[i] - S[nu][nn->N1 + i]) * (nn->x3[i] - S[nu][nn->N1 + i]); /*mse se calcula para todas las salidas*/
      if (nn->N3 == 1)
        if ((nn->x3[i] - 0.5) * (S[nu][nn->N1 + i] - 0.5) < 0.)
          *discrete_error += 1.0; /*calcula error discreto para una sola salida - problema binario*/
    }
  }

  //Sumatoria de los cuadrados de los pesos para calcular el error mediante WEIGHT DECAY
  // int t, r;
  // for (t = 1; t <= nn->N2; t++)
  //   for (int r = 0; r <= nn->N1; r++)
  //     mse_decay += nn->w1[t][r] * nn->w1[t][r];

  // for (t = 1; t <= nn->N3; t++)
  //   for (r = 0; r <= nn->N2; r++)
  //     mse_decay += nn->w2[t][r] * nn->w2[t][r];

  /*promedio los errores con la cantidad de patrones*/
  *mse /= ((float)(pat_fin - pat_ini));

  //actualizo el mse acuardo a la penalizacion dada al WEIGHT DECAY
  *mse = *mse + (nn->GAMMA * mse_decay);
...
int train(struct MLP *nn, struct DATOS *datos)
{
  ...
  decay = 1.0 - (2.0 * nn->GAMMA * eta); //WEIGHT DECAY
  ...
  for (iter = 1; iter <= nn->ITER; iter++)
  {
    ...
    for (nu1 = 0; nu1 < datos->PR; nu1++)
    {
      ...
      for (k = 1; k <= nn->N3; k++)
        nn->target[k] = datos->data[nu][k + nn->N1];
      ...
      for (i = 1; i <= nn->N3; i++)
        for (j = 0; j <= nn->N2; j++)
        {
          nn->dw2[i][j] = nn->u * nn->dw2[i][j] + eta * nn->grad3[i] * nn->x2[j]; /*termino de momentum + correcccion actual*/
          nn->w2[i][j] *= decay;                                                  //WEIGHT DECAY
          nn->w2[i][j] += nn->dw2[i][j];                                          /*correccion*/
          mse_decay += nn->w2[i][j] * nn->w2[i][j];
        }
      ...
      for (j = 1; j <= nn->N2; j++)
        for (k = 0; k <= nn->N1; k++)
        {
          nn->dw1[j][k] = nn->u * nn->dw1[j][k] + eta * nn->grad2[j] * nn->x1[k]; /*termino de momentum + correcccion actual*/
          nn->w1[j][k] *= decay;                                                  //WEIGHT DECAY
          nn->w1[j][k] += nn->dw1[j][k];                                          /*correccion*/
          mse_decay += nn->w1[j][k] * nn->w1[j][k];
        }

      /*actualizar el mse sumando el error en el patron presentado*/
      for (i = 1; i <= nn->N3; i++)
        mse += (nn->x3[i] - nn->target[i]) * (nn->x3[i] - nn->target[i]);
    ...
    if ((iter % nn->NERROR) == 0)
    {
      ...
      mse /= ((float)datos->PR);
      mse += nn->GAMMA * mse_decay; //WEIGHT DECAY
      ...
      fprintf(ferror, "%f\t%f\t%f\t%f\t", mse, mse_train, mse_decay * nn->GAMMA, mse_test);
  ...
  for (k = 0; k < datos->PTEST; k++)
  {
    for (i = 1; i <= datos->N1; i++)
      fprintf(fpredic, "%f\t", datos->test[k][i]); /*guardo las entradas de cada patron de test*/
    for (i = 1; i <= datos->N3; i++)
      fprintf(fpredic, "%f\t", datos->pred[k][i]); /*guardo la prediccion (continua) de la red para ese patron*/
    fprintf(fpredic, "\n");
  }
